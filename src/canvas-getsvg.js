//Sources:
//http://xn--dahlstrm-t4a.net/svg/libraries/canvas-svg/canvas-getsvg.js
//https://code.google.com/archive/p/canvas-svg/
//http://jsfiddle.net/pfp9y5xp/
...
CanvasSVG.Base.prototype = {
  ...
  buildPath: function(path) {
    ...
    for (var i = 0; i < path.length; i++) {
      ...
      var cmd = [subP.shift()];
      if (cmd[0] == 'A') {
        cmd.push(subP.shift() + ',' + subP.shift(), subP.shift(), subP.shift() + ',' + subP.shift(), subP.shift() + ',' + subP.shift());
      } else {
        while (subP.length) {
    ...
    switch (cmd) {
      ...
      case 'rect':
        var x = prm[0],
          ...
          w = prm[2],
          h = prm[3];
        this.path.push(['M', x, y], ['L', x + w, y], ['L', x + w, y + h], ['L', x, y + h], ['z']);
        break;
      case 'arc':
